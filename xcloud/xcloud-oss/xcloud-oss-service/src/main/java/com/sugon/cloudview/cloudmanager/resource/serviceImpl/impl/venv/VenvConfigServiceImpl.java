...
import com.sugon.cloudview.cloudmanager.vijava.environment.QueryResources.QueryResourceAnswer;
...
public class VenvConfigServiceImpl implements VenvConfigService {
	...
	public String synDataVenvConfig(VenvConfig venvconfig) throws VenvException {
		...
		List<com.sugon.cloudview.cloudmanager.vijava.data.NetPool> lstnetPool = qanswer.getNetList();
		logger.debug("QueryNetPoolAnswer，返回网络结果：" + lstnetPool.toString() + ",网络池数量：" + lstnetPool.size());
		if (lstnetPool != null && lstnetPool.size() > 0) {
			for (com.sugon.cloudview.cloudmanager.vijava.data.NetPool vijavanetPool : lstnetPool) {
				com.sugon.cloudview.cloudmanager.resource.service.bo.vnet.NetPool netPool = new com.sugon.cloudview.cloudmanager.resource.service.bo.vnet.NetPool();
		...
		if (lstresPool != null && lstresPool.size() > 0) {
			for (ResPool resPool : lstresPool) {
				...
				List<com.sugon.cloudview.cloudmanager.vijava.data.StoragePool> vijavastoragePools = resPool
						.getStoragePools();
				List<StoragePool> storagePools = new ArrayList<StoragePool>();
				if (vijavastoragePools != null && vijavastoragePools.size() > 0) {
					for (com.sugon.cloudview.cloudmanager.vijava.data.StoragePool vijavastoragePool : vijavastoragePools) {
						/**
						 * 存储池
						 */
 ...
 */
	...
	public void compareStorageAdd(ComputingPool computingPool_resource,
			List<com.sugon.cloudview.cloudmanager.vijava.data.StoragePool> vijavastoragePools, String configId)
					throws VDCException {
