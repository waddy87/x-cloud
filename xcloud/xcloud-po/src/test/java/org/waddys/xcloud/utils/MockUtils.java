 ...
 * 
 */
package org.waddys.xcloud.utils;
...
public class MockUtils {
    ...
    public static VmNetE mockVmNetE(){
    	VmNetE e = new VmNetE();
    	BeanUtils.copyProperties(mockVmNet(), e);
    	return e;
    }
    
    public static ProjectE mockProjectE(){
    	ProjectE p = new ProjectE();
    	p.setCreateTime(new Date());
    	p.setName("test-proj");
    	p.setStatus("A");
    	p.setDescription("test test");
    	p.setOrgId("123");//not null
    	return p;
    }

    public static VmHostE mockVmE() {
        VmHostE vmHost = new VmHostE();
        BeanUtils.copyProperties(mockVm(), vmHost);
        return vmHost;
    }
